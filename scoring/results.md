...
|---|---|---|---|---|---|---|---|---|---|
| `kz_slop_sonnet_v1` "Canyon Switchback" | Sonnet 5 | Claude Code subagent | all pass* | 13.5 min | ~140k | 49 | 0 | load-only | **Dry run, contaminated** — legally read ancestor repo via spec v1 loophole; used `vvis -fast` (legal at the time). Not a clean score. |
| `kz_slop_opus48_v1` "Tidewater Ascent" | Opus 4.8 | Claude Code subagent | all pass | 16.6 min | ~105k | 47 | 0 | load-only | Found + fixed own off-by-one stage-zone bug. Cheapest clean pass. |
| `kz_slop_sonnet5_v1` "Cape Ascent" | Sonnet 5 | Claude Code subagent | all pass | 25.2 min | ~199k | 92 | 0 | load-only | Read `momentum.fgd` for real entity names; validated winding + `vertices_plus` on a throwaway test map first. Explicitly flagged timer-splits gate as inferred, not witnessed. |
