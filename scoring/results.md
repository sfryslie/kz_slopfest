...
| `kz_slop_sonnet5_v4` "Refinery Spire" | Sonnet 5 | Claude Code subagent (default effort) | all pass† | 51.4 min | ~348k | 186 | 0 | load + console, 4 launches | 6 stages, 46 jumps, 660u climb, 60 props, max jump 87%. Reverse-engineered the real zonemaker 0.10.0 entity schema from exe strings (public docs/repo stale — see entry README). Found ASSETS.md prop list includes physics-only models that fail as `prop_static`; probe-compiled a verified-safe list. Explicitly declined to read foreign zone JSONs in shared install (RULES #1). Operator verified: PLAN-first ✓, generator byte-identical repro ✓, zero leaks ✓. |
